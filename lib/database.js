...
const { Database: CPPDatabase, setErrorConstructor } = require("bindings")(
  "better_sqlite3.node"
);
...
function Database(filenameGiven, options) {
  ...
  const filepath = uri ? filename.replace(/^file:(\/\/)?/, "") : filename;

  // Make sure the specified directory exists
  if (!anonymous && !fs.existsSync(path.dirname(filepath))) {
    throw new TypeError(
